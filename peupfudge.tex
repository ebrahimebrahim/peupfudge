...
Health describes a character's state of well-being. It is given entirely by the character's \emph{conditions}. 
A \emdex{condition} is a flowchart which describes a 

\subsubsection{Trauma}
\emph{Trauma} encompasses the damage caused to specific parts of a character's body by physical force. 
It can be caused by any kind of physical force (e.g. falling, combat). The trauma inflicted on a character 
should be described as a list of injuries and by what body part(s) they affect (e.g. scratched left arm, incapacitated third tentacle). 
Because these injuries may interfere with a character's ability to perform certain actions, a penalty 
should be decided for affected abilities based on how important the body part is to the ability . what is trauma? what can cause it?
describing injuries with word & body part. resultant injury penalties.
gm considers which body parts might be required for which abilities.
how injuries heal?

\subsubsection{Conditions}
what is a condition? handled by flowcharts which should be made for every desired condition.
the flowcharts have entry points, exit points, states (denoted by nodes, with a description of penalties),
and state transitions (denoted by edges, labeled with the triggering events). probabilities are dice rolls yo.
give such a bleeding example for instance. how much the flowcharts are peuped to the players could be up to the gm,
or based on for instance medical knowledge for example. peup.
...
There are four types of factors that determine the outcome of an attack. Imagine, for example, an attacker lunging with a sword at an armored defender:
\vspace{-1em}\begin{itemize}
\item \emdex{Hitting factors} (this is the name used for \emph{aiding factors} in the context of combat) are those factors which contribute to determining whether the attack misses, grazes, or hits the defender, while being dependent only on the \emph{attacker}. In the example, one hitting factor might be the sword skill of the attacker. \emdex{Preventive factors} (this is the name used for \emph{difficulty factors} in the context of combat) are those factors which contribute to determining whether the attack misses, grazes, or hits the defender, while being dependent only on the \emph{defender}. In the example, one preventive factor might be the dodging skill of the defender.
\item \emdex{Impact factors} are those factors which contribute to determining how much damage the attack inflicts on the defender, while being dependent only on the \emph{attacker}. In the example, one impact factor might be the strength skill of the attacker. \emdex{Protective factors} are those factors which contribute to determining how much damage the attack inflicts on the defender, while being dependent only on the \emph{defender}. In the example, one protective factor might be the type of armor the defender is wearing.
\end{itemize}

The following table can aid in visualizing how these four types of factors affect an attack:
...
In this case there would be no need to roll the $1$dF or consider $\NDF$.
...
\subsubsection{Impact and Protection: ($\NDF$)}\label{sec:ndf}
The damage dealt by a successful attack depends on $\NDF$, the difference between \emdex{impact} and \emdex{protection}:
$$ \NDF = impact - protection $$
