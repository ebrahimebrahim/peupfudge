...
\notes{[Does this really hold? Needs testing.]}
...
Whenever an ability level is increased, there is a chance that it's governing ability will also increase.
...
Here's how it works:
Let $A$ and $B$ be abilities such that $A$ governs $B$ with $B$ bearing weight $w$ (so $A \xrightarrow{w} B$ in the ability tree).
Whenever $B$ increases in level, the chance that $A$ increases in level is given by
$$\frac{w}{w+n}\quad,$$
where $n$ is the sum of the weights of the \emph{other} abilities governed by $A$
whose level is at most that of $A$.
We provide a table for rolling a d10, so the formula is not needed.
However, players do need to know how to compute the quantity $n$.

\notes{[provide table, and include explanation of precisely how to roll against table entries]}

\notes{[provide a quick example of determining $w$ and $n$, and include in the example an extra layer of propagation]}

Thus a high level of broad competence in an area typically comes from training various specific skills under that area.
It is easier to raise an attribute by raising many of the skills it governs above its level,
rather than by merely focusing on a few and ignoring the rest.
\notes{[Does that statement really hold true? Test it out.]}
...
\notes{suggest "character level" as root node.}

\notes{one way to make npcs more grainy is to "trim off" some leaves and turn attributes into leaves for them.}

\notes{explain what powers are and how they sit in the tree (just pretend they don't exist if off)}

\notes{examples of ability trees (or reference to later appearance of examples)}
...
\notes{on char creation you start with some xp (50 times number of skills), and use it to raise leaves as in usual development; "static" and "live" character creation (use static for classes)}

\notes{player classes can be GM-made presets}

\notes{if a skill needs to be introduced in the middle of the game due to an omission (i.e. using an attribute to roll for a particular omitted skill often), then start it at its parent -2. capped below at 1, of course.}

\notes{magic as property}
