...
\notes{[The following paragraph talks about raising skill levels only. Shouldn't it be talking about raising \emph{ability} levels?]}
...
\notes{[SOLUTION: no xp saving allowed. when xp is gained, it must be ALLOCATED. however, it can be partially spent on something, with the rest to be spent later. a player keeps track of how much xp they have invested in partially purchased skill level raises. at each xp award, only relevant skills are opened up to this allocation.]}
