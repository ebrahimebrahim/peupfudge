...
\begin{center} \includegraphics{svg/orders.eps} \end{center}
...
the whether or not $A$ increases in level.
...
\begin{center} \includegraphics{svg/tb.eps} \end{center}
...
\begin{center} \includegraphics{svg/bleeding.eps} \end{center}
...
%who is the standard being. the most typical creature type to be a PC. unitless levels are measured by them


%  the ability tree itself, including ability "difficulties" (i.e. their untrained levels)
%    are there any abilities that should be powers?
%      for each power what circumstances cause them to be enabled or disabled?
%    orders. for each ability, consider it in context of beings of the world. does it need special order label for some beings?
%    time&space. for each ability, consider various actions that could try to use it. for each action:
%      does it ever trigger rounds? under what circumstances?
%        if so, how long does it take to complete the action?
%        when does play return to normal?
%        are there any multi-component actions?
%          if so how long does each component take? does duration depend on success or failure?
%          can any components be interrupted? how, and what results?
%      does the action ever need to be resolved with location tracking?
%        if so, what are spatial parameters/constraints for the action?
%  what characteristics are relevant to game world
%  any specific properties gm wants to list? are players limited to what is in the list?
%  what are the bars? and how do they start?
%  for each creature, what health conditions are there?
%  what are inventory tree constraints?

%  want to pre-make any items? what are their characteristics/attributes/conditions?

%  xp
%  how does gm plan to enforce which skills can be trained with an xp reward?
%  how much xp do character start with during creation? shall it vary across player characters or be uniform?
%  shall gm provide any classes? (i.e. some already-trained trees to start with and reduced xp to play with). specify them.

%  quantified movement. for each creature:
%    what are main modes of locomotion? for each mode of locomotion:
%      what is base speed of a typical one of these creatures when engaging in each mode of locomotion?
%    is there an ability whose training would improve this creature's speed?
%    are speeds constrained in any other way like fatigue/ energy bar/ penalties due to health conditions/ inventory weight?

%  modules: what modules shall be used?
%  combat:
%    for each skill, how might a player use this skill in the context of an attack? for each attack:
%      is it clear which ability would be relevant to determining success of attack?
%      is it clear which ability would be relevant to determining effectiveness of attack?
%    for each creature
%      make a body diagram with possible targets of attacks. quantify relative sizes of body parts
%      decide which body parts are vital
%      choose/make a healing flowchart
%    decide on the *baseline target size*
%      (size of target that would recieve wound of degree 1 38% of time from successful unarmed attack by equally matched standard being)

%  additional things to do after answering questions:
%    design char sheet
%    put nice dice rolls on edges instead of ugly weights
%    if combat module:
%      put dice rolls on body diagram for random target selection
%      put protection modifiers on body diagram
