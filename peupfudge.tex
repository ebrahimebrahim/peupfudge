...
The basic idea is that the attempt is a success if and only if
$$ R + M \geq D, $$
where $R$ is the result of a \emph{roll}, $M$ is some sort of \emph{modifier}, and $D$ is the \emph{difficulty} of the task.
...
Summarizing, an unopposed action is successful if and only if
\begin{align*}
4\text{dF}
...
Summarizing, an opposed action is successful if and only if
\begin{align*}
&2\text{dF}
+([\text{relevant ability level}]
-[\text{attribute penalty}]
