...
Here are some suggestions for handling certain situations:
...
characters have an opportunity to choose a \emph{stance}:
defensive ($-1$), neutral ($0$), or aggressive ($1$).
This number is addeded to a character's $M$ for their attack and subtracted from their
$M'$ each time they are attacked during the round.
A neutral stance is assumed if no stance is explicitly chosen.


\paragraph{Targeting}
A player may call, before their attack is resolved, that they intend to target a certain body part.
This introduces a \emph{targeting penalty} that decreases $M$, based on the difficulty of hitting that body part.
The GM judges what the targetting penalty should be;
to give a frame of reference it could be $2$ for a torso and $5$ for an eye.
\notes{I gave little thought to those numbers. Let's discuss and find actually suitable numbers there.}
...
The damage dealt by a successful attack depends on the \emph{net damage factor} $\NDF$
