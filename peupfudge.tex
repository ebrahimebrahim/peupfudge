...
\paragraph{Grids}
Some actions \emph{trigger a grid},
...
The grid may be represented mentally or on paper or a board as needed.
The GM should decide if they want to use a square or a hex grid for their campaign.
After creating an ability tree, the GM should consider various actions
that could use the various skills and decide on the following:
\vspace{-1em}
\begin{enumerate}
\item\label{itm:grid:trigger}
Does part of this action ever need to be resolved on a grid? Under what circumstances?
...
\end{enumerate}
The grid spacing may be decided by the GM once a grid is triggered.
