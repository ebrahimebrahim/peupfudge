...
\notes{what peupfudge is all about. it gives you the classes and not the objects. what a gm is. pc vs npc. numbering system.}
...
\notes{Classes can be basically GM made presets.
...
\begin{tikzpicture}
  ...
  \node [attribute] {Mind}
    child { node [attribute] {Medicine}
      child { node [skill] {First Aid}
        edge from parent node [below] {1}}
      child { node [skill] {Wound Care}
        edge from parent node [below] {1}}
      edge from parent node [below] {1}}
    child { node [skill] {Literacy}
      edge from parent node [below] {1}}
    child { node [skill] {Engineering}
      edge from parent node [below] {1}};
\end{tikzpicture}
...
\begin{tikzpicture}
  ...
  \node [attribute] {Body}
    child { node [attribute] {Athletics}
      child { node [skill] {Running}
        edge from parent node [below] {1}}
      child { node [skill] {Swimming}
        edge from parent node [below] {1}}
      edge from parent node [below] {2} }
    child { node [skill] {Climbing}
      edge from parent node [below] {1}}
    child { node [attribute] {Muscle\\Strength} 
      child { node [skill] {Hauling}
        edge from parent node [below] {1}}
      child { node [skill] {Hand-to-Hand\\Combat}
        edge from parent node [below] {1}}
      edge from parent node [below] {2}};
\end{tikzpicture}.

Attributes are shown in blue and skills are shown in red.
%intuitive meaning (intention) of 'governing' and of weights.
...
\notes{[discuss unopposed and opposed. rolling focus on most specialized suitable ability. when rolling against attribute, subtract 2 from it's level for rolled degree purposes. this does not apply to grainy npc situation.]}
\subsection{Combat}
