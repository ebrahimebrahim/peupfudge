...
def main():
  ...
  f.write("\\begin{tabular}{l|"+(len(sizes)*"c")+"}\n")
  ...
  prot = lambda s : "$"+str(dmg.protection(size,dmg.b))+"$"
  f.write("size & ")
  for size in sizes:
    f.write(fracstr(size)+(" & " if size!=sizes[-1] else "\\\\[0.2em]\n"))
  f.write("\\cline{1-"+str(len(sizes)+1)+"}\n")
  f.write("protection & ")
  for size in sizes:
    f.write(prot(size)   +(" & " if size!=sizes[-1] else ""))
  f.write("\\end{tabular}")
  f.close()
  ...
  ds = range(max_dmg+1)
  f.write("\\begin{tabular}{l|"+(len(ds)*"c")+"}\n")
  f.write("damage &")
  for d in ds:
    f.write('$'+str(d)+("-" if d==ds[0] else "")+("+" if d==ds[-1] else "")+"$" + (" & " if d!=ds[-1] else "\\\\[0.2em]\n"))
  f.write("\\cline{1-"+str(len(ds)+1)+"}\n")
  f.write("degree of wound &")
  for d in ds:
    f.write(" $"+str(dmg.linearized_dmg(d,dmg.c,dmg.b))+"$"+  (" & " if d!=ds[-1] else "\n"))
  f.write("\\end{tabular}")
  f.close()
